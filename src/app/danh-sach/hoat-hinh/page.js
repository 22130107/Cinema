  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        ...
        try {
          const res1 = await fetch(`https://ophim1.com/v1/api/danh-sach/hoat-hinh?page=1`);
          const json1 = await res1.json();

          if (json1.status === 'success' && json1.data?.items) {
            const filteredItems = json1.data.items.filter(movie => movie.type === 'hoathinh' && movie._id && movie.name);
        ...
        for (let page = 2; page <= 10; page++) {
          promises.push(
            fetch(`https://ophim1.com/v1/api/danh-sach/hoat-hinh?page=${page}`)
              .then(res => res.json())
        ...
        results.forEach(json => {
          if (json.status === 'success' && json.data?.items) {
            const filteredItems = json.data.items.filter(movie => movie.type === 'hoathinh' && movie._id && movie.name);
            setMovies(prev => {
